...
class _EntryScreenState extends State<EntryScreen> {
  ...
  // =============== VALIDATION (ADDED) ===============
  String _purposeNameLower() {
    final p = selectedPurpose;
    final raw = (p?['purpose'] ?? p?['name'] ?? p?['title'] ?? '').toString();
  ...
  String? _validateByPurpose({required bool isWalkIn}) {
    // Base rule
    if (isWalkIn) {
      if (_isEmpty(mobileControl)) return "Kindly enter mobile number";
      if (mobileControl.text.trim().length < 8) {
        return "Kindly check your mobile number";
      }
    } else {
      if (_isEmpty(vehicleNo)) return "Kindly enter vehicle number";
    }

    // Purpose must be selected for rules to apply
    if (selectedPurpose == null) return "Kindly select purpose of visit";
    final purpose = _purposeNameLower();

    // Build required fields by purpose
    final List<String> required = [];
    if (isWalkIn) {
      required.add('mobile'); // base
    } else {
      required.add('vehicle'); // base
    }

    if (purpose.contains('delivery') || purpose.contains('coaching')) {
      required.addAll(['phone', 'unit']);
    } else if (purpose.contains('visitor')) {
      // All except IC (plus base above)
      required.addAll(
          ['name', 'email', 'contact', 'unit', 'persons', 'company', 'pass']);
    } else if (purpose.contains('contractor')) {
      // All including IC (plus base above)
      required.addAll([
        'name',
        'email',
        'contact',
    ...
    } else if (purpose.contains('dropoff') || purpose.contains('pickup')) {
      // No extra beyond base
    } else {
      // Unknown purpose → keep base only
    }
    ...
    for (final key in required) {
      switch (key) {
        case 'mobile':
        case 'phone':
          if (_isEmpty(mobileControl) || mobileControl.text.trim().length < 8) {
            missing.add('Mobile');
          }
          break;
        case 'vehicle':
          if (_isEmpty(vehicleNo)) missing.add('Vehicle');
          break;
        case 'name':
          if (_isEmpty(nameControl)) missing.add('Name');
          break;
        case 'email':
          final e = emailControl.text.trim();
          if (e.isEmpty || !_isValidEmail(e)) missing.add('E-mail');
          break;
        case 'contact':
          if (_isEmpty(contactPerson)) missing.add('Contact Person');
          break;
        case 'unit':
          if (_isEmpty(unitNumberCont)) missing.add('Unit Number');
          break;
        case 'persons':
          if (totalPerson == null) missing.add("Number of Visitor's");
          break;
        case 'company':
          if (_isEmpty(companyNoControl)) missing.add('Company Name');
          break;
        case 'pass':
          if (_isEmpty(passNoControl)) missing.add('V pass / C pass');
          break;
        case 'ic':
          if (_isEmpty(icNumberCont) || icNumberCont.text.trim().length < 5) {
            missing.add('IC Number');
          }
          break;
      }
    }
    ...
    if (missing.isNotEmpty) {
      return "Kindly fill the following: ${missing.join(', ')}";
    }

    // Optional: if user typed an email, ensure it's valid
    if (emailControl.text.trim().isNotEmpty &&
        !_isValidEmail(emailControl.text.trim())) {
  ...
  Future<void> entryFunction() async {
    // *** VALIDATION HOOK (Vehicle mode) ***
    final err = _validateByPurpose(isWalkIn: false);
    if (err != null) {
      notif('Failed', err);
      return;
    }
  ...
  // >>> FIX: make async so we can await and control _savingWalkIn reliably
  Future<void> walkIn() async {
    // *** VALIDATION HOOK (Walk-in mode) ***
    final err = _validateByPurpose(isWalkIn: true);
    if (err != null) {
      notif('Failed', err);
      return;
    }
...
//   /// Central validator.
//   /// isWalkIn=true for walk-in Save; false for vehicle-entry Save.
//   /// Returns null if OK, else an error message to show with notif().
//   String? _validateByPurpose({required bool isWalkIn}) {
//     // Base rule
//     if (isWalkIn) {
//       if (_isEmpty(mobileControl)) return "Kindly enter mobile number";
...
//     // *** VALIDATION HOOK (Vehicle mode) ***
//     final err = _validateByPurpose(isWalkIn: false);
//     if (err != null) return notif('Failed', err);
...
//     data.addAll({"in_time": DateTime.now().toString()});
//     data.addAll({"unit_no": unitNumberCont.text});
//     return Provider.of<CommonProvider>(context, listen: false).addEntry(data);
//   }
...
//                   const SizedBox(height: 24),
//                   Consumer<CommonProvider>(builder: (_, provd, __) {
//                     return provd.commonLoading2
//                         ? loading50Button()
//                         : Row(
//                             children: [
//                               SizedBox(
//                                   child: buttonWidth("Save", () {
//                                 entryFunction();
//                                 // setState(() {
//                                 //   enteringData = false;
//                                 // });
